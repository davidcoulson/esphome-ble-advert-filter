...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "components" / "ble_advert_filter"))

# Import the pure function without dragging in esphome's codegen machinery.
import ast
import types

_SRC = (Path(__file__).resolve().parent.parent / "components" / "ble_advert_filter" / "__init__.py").read_text()
